...
2. Run, from the checkout root (the directory holding `.aimem.json`):
   ...
   aimem teams setup "<TEAM>" <ROLE> --platform claude-code --platform-version <version> [--model-provider <provider> --model-id <id> --model-source runtime_reported] --label <ROLE>-<checkout directory name> --json
