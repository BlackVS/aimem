---
name: resume_team
description: "Continue the aimem team membership this checkout already holds after a client restart or compaction (the team_continue tool of the local aimem MCP server): verify or resume the session and take up the reserved attempt and unacknowledged inbox. Use only when the user asks to resume, continue or pick up team work; it never joins."
argument-hint: "[TEAM]"
allowed-tools: "mcp__aimem__team_continue mcp__aimem__team_context mcp__aimem__process_context Bash(git status *)"
