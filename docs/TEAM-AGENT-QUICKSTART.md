...
text). Any state but `ready` is an error naming the state, the cause and
the fix: `last_observed` (hub unreachable; the exact cached commit of the
selection last observed, delivered with that notice and never
authorization for a task write), `disabled`, `not_selected`, `denied`
(the hub refused the credential, or Git refused this machine; never
replaced by a cached selection), `unavailable` and `too_large` (over
32 KiB; nothing is delivered in part, templates still are).
