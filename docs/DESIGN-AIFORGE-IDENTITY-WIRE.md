...
Every verified request is audited as `team.verified`, and every refusal after authentication as `team.refused.<code>`. The actor is the authenticated user. The subject names the service, team, session, generation, role, route and correlation ID, never the handle. Refusals from this point on carry `active_mode: "team"` and the same correlation ID.
