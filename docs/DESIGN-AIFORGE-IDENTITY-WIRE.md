...
- **Lost responses.** If the hub confirms a credential but its bearer cannot be written, only that credential is revoked, and the result is reported. An unknown outcome never triggers a reissue or a guessed revocation: the CLI lists the credentials created since the request and the safe next step.
