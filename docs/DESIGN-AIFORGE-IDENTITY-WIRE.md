...
**Mapping to aicrew's `Verifier`.** `RedeemRequest.ChallengeID`, `HubID`, `Receipt` and `RequestKey` map to `challenge_id`, `hub_id`, `receipt` and `Idempotency-Key`. Aicrew's configured service ID fills the path. `VerifiedIdentity` is `identity`. Every refusal becomes a returned error, never a partial identity. Aicrew already checks that the identity is complete and that the hub matches the challenge. Retryable codes leave both stores unchanged.
