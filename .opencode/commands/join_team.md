...
1. Declare yourself honestly. Platform: `opencode`. Platform version:
   the version printed by `opencode --version` if it answers quickly, else
...
2. Run, from the checkout root (the directory holding `.aimem.json`):
   ...
   aimem teams setup "<TEAM>" <ROLE> --platform opencode --platform-version <version> [--model-provider <provider> --model-id <id> --model-source runtime_reported] --label <ROLE>-<checkout directory name> --json
