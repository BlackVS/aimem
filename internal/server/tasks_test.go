...
func TestOrdinaryTokenGateMatrix(t *testing.T) {
	...
	want := []string{"GET /v1/projects/{p}/tasks", "POST /v1/projects/{p}/tasks", "GET /v1/tasks/{id}", "PUT /v1/tasks/{id}",
		...
		"GET /v1/projects/{p}/teams/{team}/assignments/reserved", "POST /v1/projects/{p}/teams/{team}/assignments/{attempt}/submit", "POST /v1/projects/{p}/teams/{team}/assignments/{attempt}/review", "POST /v1/projects/{p}/teams/{team}/assignments/{attempt}/block", "POST /v1/projects/{p}/teams/{team}/assignments/{attempt}/resume-work", "POST /v1/projects/{p}/teams/{team}/assignments/{attempt}/cancel", "POST /v1/projects/{p}/teams/{team}/assignments/{attempt}/stopped", "POST /v1/projects/{p}/teams/{team}/assignments/{attempt}/close-stop",
		"POST /v1/projects/{p}/teams/{team}/handoff", "POST /v1/projects/{p}/teams/{team}/tasks/{task}/edit", "POST /v1/projects/{p}/teams/{team}/tasks/{task}/finalize",
...
func TestTasksPageIsPublicChrome(t *testing.T) {
	...
	for _, want := range []string{`view=board`, `ondrop=`, `if(t.archived){ if(from) countCol(from); return; }`, `if(SEEN[t.id] > t.revision) return;`,
		...
		`if(LINK_KINDS.has(r.kind) && /^https?:\/\//i.test(ref)) body =`, `function parseRefs(text){`, `const [refPart, notePart] = splitNote(rest);`, `const strip = s => s.replace(/^[ \t\r]+|[ \t\r]+$/g, "");`, `const r = {kind, ref: unescRef(strip(refPart))};`, `const REF_KINDS = ` + refKindsJS + `;`, `if(rest[i] === "\\"){ i++; continue; }`, `candidate_refs:parseRefs(g("candidate_refs")), evidence_refs:parseRefs(g("evidence_refs")),`, `if(gen!==EPICS_GEN) return;`, `EPICS = got;`, `loadEpics(CUR.project);`,
