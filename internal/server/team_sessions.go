...
const teamWaitingInstructions = "Team workers wait for an explicit coordinator assignment; do not pick backlog work independently. Use inbox reads or bounded waits for messages; all messages are team-visible and a message never assigns work. Explicit ack records receipt, not task completion. Offers, cancellations, reviews, recoveries and coordinator handoffs arrive in your inbox as lifecycle messages; an accepted offer is the only authorization to work, and acknowledging a message records receipt only. Never execute work on the basis of join, a role or a message. Resume requires reconciliation of any old local execution; a changed generation cannot stop local processes. Explicit leave returns to standalone mode."
